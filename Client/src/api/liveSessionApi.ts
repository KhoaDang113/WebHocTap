...
export const getLiveSessions = async (
  ...
  const response = await axiosClient.get<ApiResponse<LiveSessionDTO[]>>(
    "/live-sessions",
    {
      params: { status },
    },
...
export const createLiveSession = async (
  ...
  const response = await axiosClient.post<ApiResponse<LiveSessionDTO>>(
    "/live-sessions",
    payload,
  );
  return response.data.data;
...
export const getSessionsByCourseId = async (
  courseId: string,
): Promise<LiveSessionDTO[]> => {
  const response = await axiosClient.get<ApiResponse<LiveSessionDTO[]>>(
...
export const joinLiveSession = async (
  sessionId: string,
): Promise<{ session: LiveSessionDTO; token: string }> => {
