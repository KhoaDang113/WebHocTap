...
export function InstructorDashboardPage() {
  ...
  const stats = statsData || { totalCourses: 0, totalStudents: 0, totalReviews: 0, averageRating: 0 }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900">Dashboard Giảng viên</h1>
        <p className="text-slate-500 mt-2">Theo dõi và quản lý các khóa học, học viên của bạn.</p>
      </div>
      ...
      {/* Students List */}
      <h2 className="text-xl font-bold text-slate-900 mb-4">Học viên mới nhất ({students.length})</h2>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 border-b border-slate-100 text-slate-500">
              <tr>
                ...
                <th className="px-6 py-4 font-medium whitespace-nowrap">Ngày đăng ký</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {students.length === 0 ? (
                ...
                students.map((student, idx) => (
                  <tr key={`${student.userId}-${student.courseId}-${idx}`} className="hover:bg-slate-50/50">
