...
import { History, Search, Trash2, Loader2, BookOpen, CheckCircle, Clock } from 'lucide-react'
...
export default function InstructorQuizAttemptsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const instructorCourseTitles = new Set(instructorCourses.map(c => c.title))
      
      const instructorAttempts = attemptsRes.data.filter(a => instructorCourseTitles.has(a.courseTitle))
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Bạn có chắc muốn xóa lịch sử làm bài này?')) {
      try {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Quản lý và theo dõi quá trình làm bài của sinh viên trong các khóa học bạn giảng dạy.</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <div className="relative max-w-md flex-1">
            ...
            <input
              type="text"
              placeholder="Tìm theo tên SV, MSSV hoặc tên bài thi..."
              value={search}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-xs">
                          {attempt.fullName.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-semibold text-slate-800">{attempt.fullName}</p>
                      ...
                      {attempt.submitted ? (
                        <div className="flex flex-col items-center gap-1">
                          <div className="flex items-center gap-2">
                             <span className={`text-lg font-bold ${attempt.score && attempt.score >= 5 ? 'text-emerald-600' : 'text-rose-500'}`}>
                          ...
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 uppercase">
                            Hoàn thành
                          </span>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center">
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-700 uppercase">
                            Đang làm
                          </span>
                        </div>
                      )}
